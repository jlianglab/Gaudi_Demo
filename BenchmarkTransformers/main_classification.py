...
def get_args_parser():
    ...
    parser.add_option("--config", dest="config", help="yaml config file",default=None, type="string")
    parser.add_option("--output_dir", dest="output_dir", help="output dir",default=None, type="string")
    ...
    parser.add_option('--opt', default='Adam', type=str, metavar='OPTIMIZER',
    ...
    parser.add_option('--sched', default='ReduceLROnPlateau', type=str, metavar='SCHEDULER',
                        help='LR scheduler (default: "cosine"')
    parser.add_option('--lr', type=float, default=2e-4, metavar='LR',
    ...
    parser.add_option("--device", dest="device", help="cuda | hpu", default="cuda", type="string")
    ...
    parser.add_option("--torch_compile", dest="torch_compile", help="whether use torch.compile", default=False, action="callback",
                      callback=vararg_callback_bool)
    parser.add_option("--dist_url", default="env://", type=str, help="""url used to set up
...
def main(args):
    ...
    model_path = os.path.join(args.output_dir, args.data_set)
    output_path = os.path.join(args.output_dir, args.data_set)

    args.lazy_mode = os.getenv('PT_HPU_LAZY_MODE', '0') == '1'
    
    dataset_config = get_config(args.config)
    args.data_dir = dataset_config['ChestXray14']['data_dir']
    args.train_list = dataset_config['ChestXray14']['train_list']
    args.val_list = dataset_config['ChestXray14']['val_list']
    args.test_list = dataset_config['ChestXray14']['test_list']
